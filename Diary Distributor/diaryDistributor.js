console.log("Question 3: Diary Distributor");
...
function diaryDistributor(diary) {
  let orderedDiary = diary.sort((a, b) => a.valor - b.valor);
  let minValue = Math.min(...orderedDiary.map((item) => item.valor));
  let maxValue = Math.max(...orderedDiary.map((item) => item.valor));

  let arrayWithoutZero = [];

  orderedDiary.forEach((item) => {
    if (item.valor !== 0) {
      arrayWithoutZero.push(item);
    } else {
      return;
    }
  });
